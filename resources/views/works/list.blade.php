@extends('main') @section('title', ' | Genius Blog') @section('stylesheets') {{-- STYLESHEETS --}} {{ Html::style('/css/owl.carousel.css') }}
<style>
#no-projects {
    min-height: 400px;
    background: #fff;
        ...
        <section id="works-container" class="works-container works-masonry-container clearfix works-thumbnails-view">
            ...
            <section id="no-projects" class="clearfix">
                <div class="text text-center pad">
                    <h2 class="black">No projects to show</h2>
                </div>
            </section>
            @else @foreach ($projects as $project)
            <!-- start : works-item -->
            @if ( $project->type_id == 1 )
            <div class="works-item ImageWrapper works-item-one-third 
            @foreach($project->identities as $identity)
              {{ strtolower($identity->name) }}@endforeach 
            @if ( $project->type_id == 1 )zoom @else info @endif">
